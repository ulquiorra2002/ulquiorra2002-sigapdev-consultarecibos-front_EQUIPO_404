...
class MyModal extends Component {
  ...
  handlerGuardarAlumno() {
    var dataAlumno = {};
    dataAlumno.cod_alum = this.props.codigo;
    dataAlumno.nombre = this.props.nombre;
    ...
    if (dataAlumno.cod_alum !== "" ||
      ...
      fetch(url, {
        ...
        body: JSON.stringify(dataAlumno, null, 3)
      })
        .then(res => res.json())
        .then(res => {
          if (res.status) {
  ...
  handleBuscaAlumno(info) {
    let data;
    let nombrecorregido = this.props.nombre;
